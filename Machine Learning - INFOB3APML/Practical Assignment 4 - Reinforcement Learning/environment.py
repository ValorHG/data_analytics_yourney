...
class TaskEnv(gym.Env):
    ...
    def restart_run(self, position: Tuple[int,int]) -> Tuple[int,int]:
        """Resets the environment. The agent will be transferred to the starting position. The goal stays the same and the timer is set to 0.
        ...
        self.maze.objects.goal.positions = self.goal_idx
        self.timer = 0
        self.maze.objects.agent.positions = [position]
        return self.maze.objects.agent.positions[0]
